...
const getAnalytics = asyncHandler(async (req, res) => {
  ...
  // Performance: these three aggregations all read from the same Order
  // collection but don't depend on each other's results, so run them
  // concurrently instead of one-after-another — this endpoint is itself
  // one of four the dashboard fetches in parallel on load, so its own
  // internal latency directly adds to how long the dashboard spinner is
  // up.
  const [summaryResult, topItems, dailyTotals] = await Promise.all([
