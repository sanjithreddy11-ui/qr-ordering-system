...
const listTableSessions = asyncHandler(async (req, res) => {
  ...
  const sessions = await TableSession.find(filter).sort({ sessionStart: -1 }).limit(200).lean();
  res.json({ sessions });
});
...
const getSessionOrders = asyncHandler(async (req, res) => {
  ...
  const orders = await Order.find({ orderId: { $in: session.orderIds } }).sort({ placedAt: -1 }).lean();
