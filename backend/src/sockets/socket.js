...
/**
 * Permanent Order Deletion (Admin Dashboard): tells anyone already looking
 * at this order — the kitchen display, the order-success/tracking page, the
 * customer's Active Orders tab — to drop it immediately instead of waiting
 * for their own next poll/refetch.
 */
function emitOrderDeleted(order) {
  const payload = { orderId: order.orderId };
  getIO().to(`kitchen:${order.restaurantId}`).emit("order-deleted", payload);
  getIO().to(`order:${order.orderId}`).emit("order-deleted", payload);
  getIO().to(`session:${order.sessionId}`).emit("order-deleted", payload);
}
...
// Settlements Module: fired the moment "Submit Bill" locks the bill and
// moves the table out of Billing, before payment has been collected.
const emitTableAwaitingPayment = (table) =>
  emitTableEvent(table.restaurantId, "tableAwaitingPayment", table);
...
// the Tables grid both listen for this.
const emitSettlementCreated = (s) => emitTableEvent(s.restaurantId, "settlementCreated", s);
