...
export default function DiningSessionPage() {
  ...
  const handlePrintBill = async () => {
    ...
    try {
      ...
      try {
        ...
        setPrintNotice({
          ok: false,
          message: printErr instanceof PrinterError ? printErr.message : "Could not print the bill via QZ Tray.",
        });
        setPrintModal({ heading: "Bill (Before Payment)", receipt: draft, qzPrinted: false });
        return;
      }

      const { receipt: r } = await printSessionBill(tableId);
      await load();
      setPrintNotice({ ok: true, message: "Bill printed successfully." });
      setPrintModal({ heading: "Bill (Before Payment)", receipt: r, qzPrinted: true });
    } catch (err) {
  ...
  const handlePrintReceipt = async (r: ReceiptData) => {
    setBusy(true);
    setPrintNotice(null);
    try {
      await printReceiptViaQz(r, 80);
      setPrintNotice({ ok: true, message: "Receipt printed successfully." });
      setPrintModal({ heading: "Receipt", receipt: r, qzPrinted: true });
    } catch (err) {
      setPrintNotice({
        ok: false,
        message: err instanceof PrinterError ? err.message : "Could not print the receipt via QZ Tray.",
      });
      setPrintModal({ heading: "Receipt", receipt: r, qzPrinted: false });
    } finally {
      setBusy(false);
    }
  };
  ...
  if (closedReceipt) {
    return (
      <div style={{ maxWidth: 640, margin: "0 auto" }}>
        ...
        <Card style={{ textAlign: "center", padding: 40 }}>
          ...
          {printNotice && (
            <p
              style={{
                ...textStyle(12, 600),
                color: printNotice.ok ? adminColors.success : adminColors.danger,
                margin: "0 0 12px",
              }}
            >
              {printNotice.message}
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
            <TablePrimaryButton disabled={busy} onClick={() => handlePrintReceipt(closedReceipt)}>
        ...
        {printModal && (
          <ThermalReceipt heading={printModal.heading} receipt={printModal.receipt} qzPrinted={printModal.qzPrinted} onClose={() => setPrintModal(null)} />
        )}
      </div>
    );
  }
      ...
      <Card>
        ...
        {!isPaid && session.paymentMethod === "cash" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {printNotice && (
              <p style={{ ...textStyle(12, 600), color: printNotice.ok ? adminColors.success : adminColors.danger, margin: 0 }}>
                {printNotice.message}
              </p>
            )}
