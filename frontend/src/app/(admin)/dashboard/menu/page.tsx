...
const emptyForm = {
  ...
  sortOrder: "0",
  prepTimeMinutes: "10",
  // GST Management Module: "" means "use the restaurant's default GST %".
  gstSlab: "",
  hsnCode: "",
};
...
export default function AdminMenuPage() {
  ...
  // GST Management Module: the restaurant's configured slabs, fetched once
  // so the item form's GST Slab dropdown always matches whatever's set in
  // GST Management → GST Slabs, instead of a hardcoded list here.
  const [gstSlabOptions, setGstSlabOptions] = useState<number[]>([]);
  const [defaultGstPercentage, setDefaultGstPercentage] = useState<number | null>(null);
  ...
  useEffect(() => {
    loadCategories();
    fetchGstSettings(RESTAURANT_ID)
      .then((s) => {
        setGstSlabOptions(s.slabs);
        setDefaultGstPercentage(s.defaultGstPercentage);
      })
      .catch(() => {});
  }, [loadCategories]);
  ...
  const openEdit = (item: AdminMenuItem) => {
    ...
    setForm({
      ...
      gstSlab: item.gstSlab != null ? String(item.gstSlab) : "",
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        // GST Management Module: blank means "use the restaurant default".
        gstSlab: form.gstSlab === "" ? null : Number(form.gstSlab),
    ...
    <div>
      ...
      {showForm && (
        <Modal title={editing ? "Edit Item" : "Add Item"} onClose={() => setShowForm(false)}>
          ...
          {/* GST Management Module: assigns this item to one of the
              restaurant's configured GST slabs (GST Management -> GST
              Slabs). Left blank, the item bills at the restaurant's
              Default GST %. */}
          <Select
            label={`GST Slab${defaultGstPercentage != null ? ` (default: ${defaultGstPercentage}%)` : ""}`}
            value={form.gstSlab}
            onChange={(v) => setForm((f) => ({ ...f, gstSlab: v }))}
            options={[
              { value: "", label: "Use restaurant default" },
              ...gstSlabOptions.map((s) => ({ value: String(s), label: `${s}%` })),
            ]}
          />
          <TextInput
